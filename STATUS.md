...
**Security note:** `/api/debug-kv` is public (the proxy skips `/api`), writes to Redis and echoes the first 100 chars of stored classes (which contain plaintext student passcodes). Recommend deleting the route.
